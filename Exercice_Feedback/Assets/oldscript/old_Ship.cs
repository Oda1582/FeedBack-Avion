...
public class old_Ship : MonoBehaviour
{
    ...
    int Shield;
    int scraps;
    scrapsCombo CurrentScrapcombo = scrapsCombo.Mediocre;
    float TimeLastScrap;
    ...
    private void Shoot()
    {
        if(bShoot1 && NextShoot1Time < Time.time && Energy>=currentWeapon1.energyconsumption)
        {
            ...
            newbullet.GetComponent<old_bullet>().ActiveBullet(this.transform.forward,this.gameObject,currentWeapon1.Damage);
    ...
    private void ChargeShoot()
    {
        ...
        if(Energy >= currentWeapon1.energyconsumption*pourcentcharge )
        {
            ...
            newbullet.GetComponent<old_bullet>().ActiveChargedBullet(this.transform.forward, this.gameObject, Mathf.RoundToInt(currentWeapon1.Damage * pourcentcharge), Mathf.RoundToInt(currentWeapon1.CHargeSpeedMultiplier * pourcentcharge), Mathf.RoundToInt(currentWeapon1.ChargeSizeMultiplier * pourcentcharge)); ;
...
public enum old_WeaponType
{
    Classic, Charged
}
[Serializable]
public class old_weapon 
{
    public string Name;
    public int Damage;
    public float ShootRate;
    public int energyconsumption;
    public old_WeaponType type;
    ...
    public old_weapon(string name, int dmg, float shootrate)
    {
        Damage = dmg;
        ShootRate = shootrate;
