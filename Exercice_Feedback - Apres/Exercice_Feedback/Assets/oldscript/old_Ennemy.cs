...
public class old_Ennemy : MonoBehaviour
{
    ...
    private void Shoot()
    {
        if (NextShoot1Time < Time.time)
        {
            ...
            newbullet.GetComponent<old_bullet>().ActiveBullet(this.transform.forward, this.gameObject, currentWeapon.Damage);
    ...
    public void TakeDamage(int dmg, GameObject emmiter)
    {
        ...
        if(life<=0)
        {
            emmiter.GetComponent<old_Ship>().EnnemyDestroyed();
